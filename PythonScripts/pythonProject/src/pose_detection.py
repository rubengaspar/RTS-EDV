...
class PoseDetector:
	...
	def extract_keypoints(self, results):
		...
		if results.pose_landmarks:
			...
			kp = {k: {'x': v.x, 'y': v.y, 'z': v.z, 'visible': v.visibility > 0.5} for k, v in
			             selected_keypoints.items()}
		return kp
	...
	def draw_keypoints(self, frame, faces, results, keypoints):
		...
		if results.pose_landmarks:
			...
			for key, point in keypoints.items():
				if point['visible']:
					cv2.circle(frame, (int(point['x'] * frame.shape[1]), int(point['y'] * frame.shape[0])), 10,
					           (0, 255, 0), -1)
